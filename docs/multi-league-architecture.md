...
| Hardening (review) | Yes | Migration `20260926180000_hardening.sql`: membership writes need the league context (separate select and write policies), `leagues_member` counts active memberships only, `users` updates and inserts are limited to the columns the API writes (so `is_admin` is SQL-only), and a legacy `(fixture_id, kind)` index keeps the previously deployed API's milestone upsert working through the deploy. Service fixes: steward routes reject membership ids from other leagues; withdrawal stops mark accrual instead of zeroing it (the reason "Member withdrawn" is reserved); claim, duty creation and first sign-in handle races; emblem uploads must match their content type; a released name drops its team and read state; an admin acting through a membership is audited as "name (admin)". |
