...
- `apps/api`: FastAPI app with `GET /v1/health`; the competition routes under `/v1/competitions/{competitionId}` (`matches/{fixtureId}`: teamsheets, kickoff forecast and live score timeline for one fixture, cached in PostgreSQL; `rounds/{round}/scores`: live scores for a round; `matches/{fixtureId}/preview`: the Pavilion match preview; `rounds/{round}/updates`: teamsheets, previews, kick-offs and results for the notifications panel); the account routes `GET /v1/me` (the account and its leagues) and `GET`/`POST /v1/join/{code}` (join a league by its code); the league endpoints under `/v1/leagues/{leagueId}` (me, members with withdrawal and reinstatement, Superbru standings, duties, marks, evidence, feed, and for the captain or admin the emblem and accent colour (`emblem/uploads`, `appearance`) and the join code (`join-code/rotate`, `DELETE join-code`)) behind Supabase Auth token verification; `GET /v1/competitions` (the competition registry); the admin's management centre under `/v1/admin` (`GET`/`POST leagues`, `PATCH leagues/{leagueId}` to rename, archive or restore, `POST leagues/{leagueId}/captain` to appoint a captain, `POST leagues/{leagueId}/members/me` to join a league outside the season); and the preview agent's token-protected `/v1/agent` routes. See [apps/api/README.md](apps/api/README.md) for the bootstrap command and Storage setup.
