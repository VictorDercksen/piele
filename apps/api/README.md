...
- League endpoints, all requiring a Supabase access token from an active member (`Authorization: Bearer …`): `GET /v1/me` (includes the member's favourite team and a short-lived photo URL), `POST /v1/me/photo/uploads` (photo upload grant) and `PUT /v1/me/profile` (own favourite team and photo); `GET /v1/members`, `POST /v1/members`, `PATCH /v1/members/{id}` (reserve a name for an email) and `POST /v1/members/{id}/release` (captain); `GET /v1/duties?round=`, `POST /v1/duties` and `POST /v1/duties/{id}/void` and `POST /v1/duties/{id}/reset-clock` (captain), `GET /v1/duties/default-deadline`; `GET /v1/marks`; `GET /v1/standings?round=` (Superbru round points, ranked; tied points share a rank) and `PUT /v1/rounds/{round}/standings` (captain; replaces the round's table from the pool results, body `{"standings": [{"memberId", "points"}]}`, points in steps of 0.1); `POST /v1/evidence/uploads` (upload grant), `POST /v1/evidence` (publish a submission), `POST /v1/evidence/links/{id}/decision` (uninvolved captain), `GET /v1/evidence/assets/{id}/playback`; `GET /v1/feed?round=`; `PUT /v1/me/notifications` (what the member has read in the notifications panel: a high-water mark `readAt` and up to 200 `readKeys` read individually above it, merged so a stale device never moves the mark back; `GET /v1/me` returns them). Every mutation writes its audit event and feed entry in the same transaction. See `app/league/`.
...
- `GET /v1/rounds/{round}/updates` (active member): the round's competition events for the notifications panel, in time order: `teamsheets_published`, `preview_published` (with the revision), `kicked_off` (at the published kickoff time once it has passed, or earlier when the score feed shows the match under way; a feed outage never hides a kick-off, and `postponed` or `cancelled` suppresses it) and `full_time` (with the score). Teamsheets are looked for from three days before kickoff to two days after; the first time the API sees them published, or sees full time, it records the moment in `piele.fixture_milestones`, so the times stay put while provider snapshots expire and are fetched again. Uses the same cached snapshots as the match centre, so a round's request adds no provider calls beyond what the match pages make. The route resolves the member and reads its rows in one short transaction, calls the providers with no transaction open, then records new milestones in a second one: the runtime pool holds a single connection that the snapshot cache also needs, so a request that keeps its transaction through provider calls stalls the whole API for the 30-second pool timeout.
