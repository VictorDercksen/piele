...
- `GET /v1/matches/{fixtureId}/preview` (active member): the latest Pavilion preview for the fixture, or `preview: null`. Summary, key factors and mood per side, and the cited sources.
- `GET /v1/rounds/{round}/updates` (active member): the round's competition events for the notifications panel, in time order: `teamsheets_published`, `preview_published` (with the revision), `kicked_off` (at the kickoff time, once the score feed shows the match under way or finished) and `full_time` (with the score). Teamsheets are looked for from three days before kickoff to two days after; the first time the API sees them published, or sees full time, it records the moment in `piele.fixture_milestones`, so the times stay put while provider snapshots expire and are fetched again. Uses the same cached snapshots as the match centre, so a round's request adds no provider calls beyond what the match pages make.
