...
Piele administers a private URC league. The authoritative implementation plan is ../../../piele-application-plan.md relative to this directory. User decisions take precedence over proposed defaults. This directory holds a uv-managed FastAPI service: app/main.py (Vercel entrypoint, /v1 routes, exact-origin CORS, X-Request-ID and Cache-Control: no-store middleware), app/config.py (pydantic-settings), app/db.py (lazy SQLAlchemy/psycopg engine for the Supabase transaction pooler), GET /v1/health and GET /v1/matches/{fixtureId}. The match centre lives in app/matchcentre: the bundled schedule (app/data/urc_fixtures.json), club and stadium catalogues, a read-through snapshot cache backed by the `piele.external_snapshots` table or process memory without a database, and providers for URC teamsheets and Open-Meteo. The URC lineup query is unverified against the live feed.

The league domain lives in app/league: auth.py verifies Supabase access tokens (JWKS, or the legacy secret when configured); context.py resolves the actor inside one transaction per request, setting transaction-local `piele.auth_subject`, `piele.auth_email` and `piele.league_id` for row level security and claiming a membership by verified invited email once; tables.py mirrors supabase/migrations/20260924160000_league_foundation.sql (users, leagues, league_memberships, seasons, season_memberships, duties, media_assets, evidence_submissions, duty_evidence_links, feed_entries, audit_events) with SQLAlchemy Core; marks.py is the authoritative house-mark calculation; storage.py issues Supabase Storage upload grants and playback URLs with the service role key (REST shapes unverified against a live bucket); service.py holds the domain operations, each writing its audit event and feed entry in the actor's transaction; bootstrap.py is the operator command that creates the league, captain and season. app/routers/league.py exposes them under /v1. User decisions recorded in this build: Spoon duties default to the next round's first kickoff; a member's accepted evidence completes the duty at submission time, while evidence the captain records for a member completes it at the captain-entered time; the captain cannot decide evidence for their own duties; Spoon duties from the future Superbru sync will be proposed for captain confirmation, not created automatically. Pause intervals, cases, voting, invitations by token and captain transfer are not built. Do not claim otherwise. See README.md for local run, bootstrap, Supabase and Vercel setup.
