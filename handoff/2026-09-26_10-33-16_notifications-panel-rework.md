...
- `src/styles/ui.scss`: `.league .spoon-duty .text-button` gave every text button on a spoon duty card cream ink, which has been unreadable on the pale textured button pill since the rugby button restyle (the panel's pinned spoon duty, the duties page's "Void duty" and "Reset clock", the captain's review row). The cream now applies only to plain text links (`a.text-button`, like the home card's "Duty details"); buttons keep their dark ink. Checked by screenshot in the panel, on the duties page and on the home card.
